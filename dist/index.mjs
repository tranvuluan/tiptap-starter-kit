...
import { markInputRule, markPasteRule, isNodeSelection, posToDOMRect, findParentNode, mergeAttributes, Node, findChildren, wrappingInputRule, defaultBlockAt, InputRule, nodeInputRule, textblockTypeInputRule, Extension, isActive, isTextSelection } from '@tiptap/core';
import { Code as Code$1 } from '@tiptap/extension-code';
import { Link as Link$1 } from '@tiptap/extension-link';
import { Plugin, PluginKey, Selection, EditorState, TextSelection, NodeSelection } from '@tiptap/pm/state';
...
import { TaskItem as TaskItem$1 } from '@tiptap/extension-task-item';
import { Table as Table$1 } from '@tiptap/extension-table';
import { Decoration, DecorationSet, EditorView } from '@tiptap/pm/view';
import { TableRow as TableRow$1 } from '@tiptap/extension-table-row';
import { TableCell as TableCell$1 } from '@tiptap/extension-table-cell';
import { TableHeader as TableHeader$1 } from '@tiptap/extension-table-header';
...
import { Mark, DOMSerializer, Slice } from '@tiptap/pm/model';
...
class FloatMenuView {
    ...
    _popover() {
        ...
        return tippy(document.body, this.options.tippy ? this.options.tippy({
            options,
            view: this,
            editor: this.editor
        }) : options);
...
const Link = Link$1.extend({
    ...
    addProseMirrorPlugins () {
        return [
            ...Link$1.config.addProseMirrorPlugins?.apply(this) ?? [],
            new Plugin({
...
const HardBreak = HardBreak$1.extend({
    name: "hardBreak",
    addStorage () {
        return {
            ...this.parent?.(),
...
const Table = Table$1.extend({
    ...
    addProseMirrorPlugins () {
        return [
            ...Table$1.config.addProseMirrorPlugins?.apply(this) ?? [],
            new Plugin({
                key: new PluginKey(`${this.name}-float-menu`),
                view: FloatMenuView.create({
                    editor: this.editor,
                ...
                props: {
                    decorations: (state)=>{
                        ...
                        if (this.editor.isEditable) {
                            ...
                            if (cell) {
                                decorations.push(Decoration.widget(cell.pos + 1, ()=>{
...
const TableRow = TableRow$1.extend({
    ...
    addProseMirrorPlugins () {
        return [
            ...TableRow$1.config.addProseMirrorPlugins?.apply(this) ?? [],
            new Plugin({
                key: new PluginKey(`${this.name}-float-menu`),
                view: FloatMenuView.create({
                    editor: this.editor,
                        ...
                        if (this.editor.isEditable) {
                            ...
                            if (cells) {
                                for(let index = 0; index < cells.length; index++){
                                    decorations.push(Decoration.widget(cells[index].pos + 1, ()=>{
                                        const grip = document.createElement("div");
...
const TableCell = TableCell$1.extend({
    ...
    addProseMirrorPlugins () {
        return [
            ...TableCell$1.config.addProseMirrorPlugins?.apply(this) ?? [],
            new Plugin({
                key: new PluginKey(`${this.name}-float-menu`),
                view: FloatMenuView.create({
                    editor: this.editor,
                        ...
                        if (this.editor.isEditable) {
                            ...
                            if (table) {
                                ...
                                for (const pos of map.cellsInRect({
                                    ...
                                    decorations.push(Decoration.widget(table.start + pos + 1, ()=>{
                                        const grip = document.createElement("div");
...
const TableHeader = TableHeader$1.extend({
    ...
    addProseMirrorPlugins () {
        return [
            ...TableHeader$1.config.addProseMirrorPlugins?.apply(this) ?? [],
            new Plugin({
                key: new PluginKey(`${this.name}-float-menu`),
                view: FloatMenuView.create({
                    editor: this.editor,
...
class BlockMenuView {
    ...
    onStart(props) {
        ...
        this._popover = tippy(document.body, this.options.tippy ? this.options.tippy({
            options,
            view: this,
            editor: this.editor
        }) : options);
...
const Image = Image$1.extend({
    ...
    addProseMirrorPlugins () {
        return [
            ...Image$1.config.addProseMirrorPlugins?.apply(this) ?? [],
            new Plugin({
                key: new PluginKey(`${this.name}-float-menu`),
                view: FloatMenuView.create({
                    editor: this.editor,
...
const Mermaid = Node.create({
    ...
    addNodeView () {
        const render = debounce(300, (code, node)=>{
            ...
            mermaid.render(dom.id, code).then(({ svg, bindFunctions })=>{
                dom.innerHTML = svg;
                bindFunctions?.(dom);
...
const Markdown = Extension.create({
    ...
    onBeforeCreate () {
        // processor
        this.storage.processor = unified().use(remarkParse).use(remarkStringify).use(remarkGfm).use(remarkDirective);
...
const BlockMenu = Extension.create({
    name: "blockMenu",
    addOptions () {
        return {
            items: [
...
const ClickMenu = Extension.create({
    name: "clickMenu",
    addProseMirrorPlugins () {
                ...
                props: {
                    handlePaste: (view, event)=>{
                        ...
                        if (isMarkdown(text)) {
                            ...
                            view.dispatch(view.state.tr.replaceSelection(new Slice(slice.content, contentSlice.openStart, contentSlice.openEnd)));
                            return true;
                        }
                        return false;
                    },
...
const FloatMenu = Extension.create({
    name: "floatMenu",
    addOptions () {
        return {
            items: [
...
export { Audio, BlockMenu, BlockMenuView, Blockquote, Bold, BulletList, ClickMenu, ClickMenuView, Clipboard, Code, CodeBlock, Details, DetailsContent, DetailsSummary, Document, Embed, Emoji, FloatMenu, FloatMenuView, HardBreak, Heading, Highlight, HorizontalRule, Image, InnerEditorView, InnerRenderView, InnerResizerView, Italic, Link, ListItem, Markdown, MathBlock, MathInline, Mermaid, OrderedList, Paragraph, StarterKit, Strike, Subscript, Superscript, Table, TableCell, TableHeader, TableRow, TaskItem, TaskList, Text, Underline, Uploader, Video, colors, configure, debounce, findTable, getCellInTable, getCellsInColumn, getCellsInRow, icon, isCellSelection, isColumnSelected, isInCode, isMarkdown, isRectSelected, isRowSelected, isTableSelected, parseAttributes, remarkDecoration, selectColumn, selectRow, selectRowOrColumn, selectTable, setAttributes, unwrap, wrap };
